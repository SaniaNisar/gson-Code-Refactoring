...
// * </pre>
// *
// * It's possible to specify different type adapters on a field, that field's type, and in the
// {@link
...
// * JsonSerializer}. Using {@link TypeAdapterFactory} makes it possible to delegate to the
// enclosing
...
// *   <li>Otherwise, if the adapter class has a no-args constructor (regardless of which
// visibility),
...
// * annotation. It is not guaranteed that a new adapter is created every time the annotated class
// or
 ...
 * </pre>
 *
 * <p>Since {@code User} class specified {@code UserJsonAdapter.class} in {@code @JsonAdapter}
 * annotation, it will automatically be invoked to serialize/deserialize {@code User} instances.
 *
 * <p>Here is an example of how to apply this annotation to a field.
 *
 ...
 * </pre>
 *
 * <p>It's possible to specify different type adapters on a field, that field's type, and in the
 * {@link GsonBuilder}. Field annotations take precedence over {@code GsonBuilder}-registered type
 * adapters, which in turn take precedence over annotated types.
 *
 * <p>The class referenced by this annotation must be either a {@link TypeAdapter} or a {@link
 * TypeAdapterFactory}, or must implement one or both of {@link JsonDeserializer} or {@link
